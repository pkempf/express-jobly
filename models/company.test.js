...
describe("findByCriteria", function () {
  test("works: all filters", async function () {
    let filters = { nameLike: "c", minEmployees: 2, maxEmployees: 3 };
    let companies = await Company.findByCriteria(filters);
    expect(companies).toEqual([
      {
        handle: "c2",
        name: "C2",
  ...
  test("bad request if minEmployees > maxEmployees", async function () {
    try {
      let filters = { minEmployees: 100, maxEmployees: 50 };
      let companies = await Company.findByCriteria(filters);
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
