...
class Company {
  ...
  /** Find companies by criteria, designing a custom filter string
   *  depending on which criteria are given.
   *
   *  Takes filter object; not all fields required: {nameLike, minEmployees, maxEmployees}
   *
   *  Returns [{ handle, name, description, numEmployees, logoURL }, ...]
   */
  ...
  static async findByCriteria(filters) {
    ...
    if (!(nameLike || minEmployees || maxEmployees)) return Company.findAll();
    ...
    if (minEmployees) {
      if (values.length > 0) {
        filterString += " AND num_employees >= $" + (values.length + 1);
      } else {
        filterString += "num_employees >= $1";
      }
